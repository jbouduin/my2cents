...
(function() {
  Notification.requestPermission().then(function(status) {
    if (status === 'granted') {
      var script = document.querySelector('script[data-m2c-target]');
      const url = new URL(document.URL);
      my2cents_host = `${url.protocol}//${url.host}/${script.dataset.m2cRoot}`;
      serviceWorkerName = `${my2cents_host}/embed/sw.js`;
      fetch(
        my2cents_host + '/vapidData',
        {
          method: 'GET',
...
function subscribe() {
  navigator.serviceWorker.ready.then(function(reg) {
    ...
    const applicationServerKey = urlB64ToUint8Array(appSrvPK);
...
function removeSubscriptionFromServer(endpoint) {
  ...
  fetch(my2cents_host + '/unsubscribe', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ publicKey: encodedKey, auth: encodedAuth, endpoint })
  })
